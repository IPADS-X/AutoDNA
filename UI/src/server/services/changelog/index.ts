...
export class ChangelogService {
  ...
  async getPostById(id: string, options?: { locale?: Locales }) {
    ...
    try {
      ...
      console.log('[ChangelogFetch]fetching changelog pre', id, url);
      const response = await fetch(url, {
        next: { revalidate: 3600, tags: [FetchCacheTag.Changelog] },
      });
      console.log('[ChangelogFetch]fetched changelog post', id, url);
      ...
      console.warn('[ChangelogFetchError]failed to fetch changlog post', id);
      console.warn(e);
