...
import { change_to } from '@/custom/routing';
...
export interface ConfigCellProps {
  ...
  threadId?: string;
  title: string;
  index: number; // <-- 1. Add index to the component's props
}

const TopicItem = memo<ConfigCellProps>(({ title, active, id, fav, threadId, index }) => { // <-- 2. Receive index as a prop
  ...
  const backgroundColor = active ? '#fbb584' : undefined;

  return (
    <Flexbox style={{ position: 'relative', backgroundColor: backgroundColor }}
    >
      <Flexbox
        align={'center'}
        ...
        horizontal
        onClick={() => {
          // Does not support clicking the first item
          if (index > 0) {
            console.log(`Clicked item at index: ${index} with title: "${title}"`);
            change_to(index);
          }
        }}
        onMouseEnter={() => {
