...
namespace CYStandardProcedure
{
    public partial class DIForm : Form
    {
        ...
        { 
            ...
            timer1.Enabled = true;
            /***得到总输入点个数***/
            mTotalInput = IOConfig.Instance.InputNames[0].Count;
            int modvalue = 0;
            modvalue = mTotalInput % 48;
            /***得到输入点页数***/
            if (mTotalInput > 48)
            {
                /***输入总数是48整数***/
                if (modvalue == 0)
                {
                    mTotalPage = mTotalInput / 48;
                }
                /***输入总数不是48整数***/
                else
                {
                    mTotalPage = (mTotalInput - modvalue) / 48 + 1;
                }
            }
            /***不满48只有一页***/
            else
            ...
            /***订阅语言改变事件***/
            LanguageConfig.Instance.LanguageChangeEvent += DIForm_LanguageChangeEvent; ;
            /***触发语言改变事件***/
            LanguageConfig.Instance.ChangeLanguage(LanguageConfig.Instance.Language);
        }

        private void DIForm_LanguageChangeEvent(string strLanguage)
        {
            if(strLanguage=="CH")
            {
                mIndex = 0;
            ...
            try
            {
                ...
                foreach (Control con in this.Controls)
                {
                    if (PageDic[mCurPage] == 48)
                    {
                        ...
                        {
                            (con as Label).Image = IOConfig.Instance.InputsStatus[int.Parse(con.Name.Substring(5)) + (mCurPage - 1) * 48] ? imageList1.Images[1] : imageList1.Images[0];
                        }
                    }
                    else
                    {
            ...
            foreach (Control con in this.Controls)
            {
                if (PageDic[mCurPage] == 48)
                {
                    if (con.Name.Contains("label"))
                    {
                        con.Enabled = true;
                        con.Visible = true;
                        con.Text = IOConfig.Instance.InputNames[mIndex][int.Parse(con.Name.Substring(5)) + (mCurPage - 1) * 48];
                    }
                }
                else
                {
