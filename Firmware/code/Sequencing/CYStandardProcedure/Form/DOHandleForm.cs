...
{
    public partial class DOHandleForm : Form
    {
        ...
        private void DOHandleForm_Load(object sender, EventArgs e)
        {
            ...
            /***订阅语言改变事件***/
            LanguageConfig.Instance.LanguageChangeEvent += DOHandleForm_LanguageChangeEvent; ;
            /***触发语言改变事件***/
            LanguageConfig.Instance.ChangeLanguage(LanguageConfig.Instance.Language);
        }

        private void DOHandleForm_LanguageChangeEvent(string strLanguage)
        {
            if(strLanguage=="CH")
            {
                mIndex = 0;
            ...
            try
            {
                ...
                foreach (Control con in this.Controls)
                {
                    if (PageDic[mCurPage] == 48)
                    {
                        ...
                        {
                            (con as Label).Image = IOConfig.Instance.OutputStatus[int.Parse(con.Name.Substring(5)) + (mCurPage - 1) * 48] ? Properties.Resources.Switch_On : Properties.Resources.Switch_Off;
                        }
                    }
                    else
                    {
        ...
        private void Switch_Click(object sender, EventArgs e)
        {
            ...
            string str = Enum.GetNames(typeof(_OutputCollect))[int.Parse(lab.Name.Substring(5)) + (mCurPage - 1) * 48];
            if (IOConfig.Instance.OutputStatus[int.Parse(lab.Name.Substring(5)) + (mCurPage - 1) * 48])
            {
                IOConfig.Instance.SetSingleOut(str, 0);
            }
            else
            {
                IOConfig.Instance.SetSingleOut(str, 1);
            }
        }
