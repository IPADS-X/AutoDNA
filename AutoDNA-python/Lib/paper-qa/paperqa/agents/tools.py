...
class PaperSearch(NamedTool):
    ...
    async def paper_search(
        ...
        for r in results:
            ...
            # mark Modified here for printing out the papers found
            logger.info("Below is info of the paper")
            # authors = getattr(this_doc_details, "authors", [])
            # authors_str = ', '.join(authors) if authors else "Not Available"
            if isinstance(this_doc_details, DocDetails):
                logger.info(
                    f"THis is my version. Title: {this_doc_details.title}, Docname: {this_doc_details.docname}, DocKey: {this_doc_details.dockey},"
                )
                if this_doc_details.file_location:
                    logger.info(f" File Location: {this_doc_details.file_location}")
            logger.info("---------")
