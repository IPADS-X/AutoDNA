...
class Docs(BaseModel):  # noqa: PLW1641  # TODO: add __hash__
    ...
    async def aquery(  # noqa: PLR0912
        ...
        # twetrttr: remove duplications
        unique_contexts = {}
        for c in contexts:
            if c.text.name not in unique_contexts:
                unique_contexts[c.text.name] = c
            # if it's in, check if has a better score, update the score
            elif c.score > unique_contexts[c.text.name].score:
                unique_contexts[c.text.name] = c
