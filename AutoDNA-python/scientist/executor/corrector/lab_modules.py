...
class Container:
    ...
    def __init__(self, label: str, index: int, capacity: ContainerType = ContainerType.P200, volume: float = 0.0, replenishable: bool = False, location: Optional[Location] = None):
        ...
        self.location = location
        # Default group 0 means not part of any specific capped strip
        self.strip_group_id = 0
...
class Capper_200ul_tubes(BaseMachine, abc.ABC):
    ...
    # Static counter to generate unique strip IDs for each capping cycle
    _next_strip_group_id = 1
    ...
    def cap(self) -> None:
        ...
        # Assign a new unique strip ID to all containers currently on the capper
        current_strip_id = Capper_200ul_tubes._next_strip_group_id
        Capper_200ul_tubes._next_strip_group_id += 1
        
        for container in self.containers_on_instrument:
            container.strip_group_id = current_strip_id
            container.capped = True
...
class Robot(BaseMachine, abc.ABC):
    ...
    def move_container(self, container: Container, destination: Location) -> None:
        ...
        # --- Validation for Fluorometer ---
        if destination.description in INSTRUMENT_REGISTRY:
            dest_machine = INSTRUMENT_REGISTRY[destination.description]
            
            # Check if we are moving to the Fluorometer
            if isinstance(dest_machine, Fluorometer200ul):
                # Collect strip IDs from all containers currently on the fluorometer
                # plus the container we are about to move there.
                all_ids = {c.strip_group_id for c in dest_machine.containers_on_instrument}
                all_ids.add(container.strip_group_id)
