...
from config import tool_max_length_output, settings, current_output_file_path, protocol_mode
import pickle
from typing import (
    Any,
    Callable,
...
class AgentAction(Enum):
    """
    Protocol Agent: Procedure Design
Reagent Agent: Reagent Check
Protocol Agent: Procedure Revision
Code Agent: Code Generation
Code Agent: Code Correction
Hardware Agent: Code Execution
    """
    PROCEDURE_DESIGN = "Procedure Design"
    REAGENT_CHECK = "Reagent Check"
...
class MyToolNode(RunnableCallable):
    ...
    def _run_one(
        ...
        try:
            ...
            output_dir = os.path.join(output_path, self.graph_name)
            os.makedirs(output_dir, exist_ok=True)
            output_pickle = os.path.join(output_dir, f"step{step_id}-{call['name']}-output.pkl")
            output_json = os.path.join(output_dir, f"step{step_id}-{call['name']}-output.json")
            output_md = os.path.join(output_dir, f"step{step_id}-{call['name']}-output.md")

            if not os.path.exists(output_pickle):
                tool_input = {**call, **{"type": "tool_call"}}
                input_file = output_dir + f"/step{step_id}-{call['name']}-input.json"
                json.dump(tool_input, open(input_file, "w"))
            ...
            if call['name'] not in ["Code"]:
                action_str_prefix = f"{call['name']} Agent: "
                action_str = action_str_prefix
                if call['name'] == "Protocol":
                    action_str = f"{action_str_prefix}{protocol_mode.mode}"
                if call['name'] == "Reagent":
                    action_str = f"{action_str_prefix}Reagent Check"
                if call['name'] == "Hardware":
                    action_str = f"{action_str_prefix}Code Execution"
        ...
        if isinstance(response, Command):
            ...
            if len(response.content) > tool_max_length_output:
                response.content = response.content[:tool_max_length_output] + "..."
            return response
        else:
            raise TypeError(
