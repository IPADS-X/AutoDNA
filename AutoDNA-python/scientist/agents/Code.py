...
def no_instrument_invoke(scheme: str, multiple_inputs: str = "") -> str:
    # Get external tools usage
    with open(external_libs_usage_path, "r", encoding='utf-8') as f:
        external_libs = f.read()
    with open(reference_list_path, "r", encoding='utf-8') as f:
        references = f.read()
    full_prompt = prompt_no_instrument.format(coder_hints=coder_prompt_hints_pure_software, scheme=scheme, external_libs=external_libs, references=references)
    if multiple_inputs != "":
    ...
    cleaned_code = extract_python_code(llm_response)
    file_id = file_manager.add_file(AgentOutputPrefix.PDA_OUTPUT, cleaned_code)
