...
from config import output_path, settings, input_manual_mappings_path, manual_path, protocol_mode, external_libs_list_path
from prompts.agents.Protocol.prompt import *
from prompts.agents.Code.prompt import extractor_prompt_extract
import os
...
from tools.utils import WorkflowCache, getLatestExperimentWorkflowByCache, CoflowCache
...
def choose_mode_by_LLM(file_ids: list[str], manual_ids: list[str]) -> WorkflowType:
    ...
    1. If there is no previous {AgentOutputPrefix.PGA_OUTPUT} output in the file IDs, then it's the initial procedure generation. Output "Initial"(without quotes) AND STOP. Otherwise, continue to the next step.
    2. If there is a new manual, then it's the refinement mode. Output "Refinement"(without quotes) AND STOP. Otherwise, continue to the next step.
    3. If there is {AgentOutputPrefix.HPA_OUTPUT} in the file IDs or in the history input IDs, then it's the optimizing mode. Output "Optimizing"(without quotes) AND STOP. Otherwise, continue to the next step.
    ...
    You must execute the rules in order.
    """
    )
...
def get_relevant_manual_content(manual_paths: list[str], experiment_name: str) -> str:
    """
    Iterates through manual paths, extracts text, and uses LLM to filter for relevance 
    to the specific experiment goal.
    """
    manual_text = ""
    relevance_prompt = PromptTemplate.from_template(
"""
Analyze the following manual content and determine if it is relevant to the experiment: {experiment_name}.
Output "YES" if it is relevant, "NO" otherwise. Output only one word.
...
"""
    ...
    for path in manual_paths:
        ...
        single_manual_text = extract_all_from_pdfs([path])
        
        # Invoke LLM to check relevance
        check_res = workflow_model.invoke(relevance_prompt.format(
            experiment_name=experiment_name, 
            text=single_manual_text
        )).content.strip()
        
        if "YES" in check_res.upper():
            logger.debug(f"Manual at {path} is relevant.")
...
def Protocol(experiment_name: str, pure_software = False, retry = False, file_ids: list[str] = [], manuals: list[str] = []):
    ...
    Args:
        ...
        pure_software (bool, optional): A flag indicating only involves software, judged by the experiment goal.
...
""" + f"\n{PGA_prompt_intermediate_reagents}" 
    ...
    if category == WorkflowType.ALGORITHM:
        ...
        # read extenal libs list
        with open(external_libs_list_path, "r") as f:
            external_libs_list = f.read()
        final_prompt = prompt_generate_scheme.format(user_prompt=user_prompt, text=context, experiment_name=experiment_name, tools_list=external_libs_list)
    ...
    if category == WorkflowType.INITIAL:
        ...
        # --- Use helper function for manual filtering ---
        manual_text = get_relevant_manual_content(manual_paths, experiment_name)
        # ------------------------------------------------
    ...
    CoflowCache.update_best_protocol(f_id)
