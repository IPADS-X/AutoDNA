...
from config import output_path,  paper_path, qa_prompt_suffix, qa_prompt_option, input_path
...
def answer_one_question(question, suffix=None, mock=False, save=False, filter=None):
    ...
    # if question in AnswerCache.answers:
    #     return AnswerCache.get_answer(question)
    ...
    settings = Settings(
        ...
        agent=AgentSettings(
            agent_llm=model,
            index_concurrency=3),
        paper_directory=os.path.join(paper_path, "final"),
        use_absolute_paper_directory=False,
        # e.g., papers/
        embedding="gemini/text-embedding-004",  
        # e.g., st-nomic-ai/nomic-embed-
    ...
    settings.agent.return_paper_metadata = False
    #settings.texts_index_mmr_lambda = 0.95
