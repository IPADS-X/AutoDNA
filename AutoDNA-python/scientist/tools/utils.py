...
class CoflowCache:
    ...
    @classmethod
    def update_best_protocol(cls, protocol_id: str):
        cls.best_protocol = protocol_id

    @classmethod
    def get_best_protocol(cls) -> str:
        return cls.best_protocol
    
    @classmethod
    def get_cache_dir(cls) -> str:
        return cls.coflow_dir
    
def get_inventory(with_intermediate=False):
    ...
    if with_intermediate:
        ...
        if not os.path.exists(os.path.join(reagent_path, "reagent_intermediate.json")):
            return inventory
        with open(os.path.join(reagent_path, "reagent_intermediate.json"), "r", encoding='utf-8') as f:
            intermediate_reagents = json.load(f)
            # if empty, return
            if not intermediate_reagents:
                return inventory
            inventory.extend(intermediate_reagents)
    return inventory
...
def insert_intermediate_reagents(reagent_names: list) -> bool:
    # Intermediate reagent file path
    intermediate_file = os.path.join(reagent_path, "reagent_intermediate.json")

    # Only support latest intermediate products for now
    # Previous intermediate products will be cleaned up
    
    entries = []
    for name in reagent_names:
        entry = {}
        entry['id'] = 9000 + len(entries) + 1
        entry['name'] = name
