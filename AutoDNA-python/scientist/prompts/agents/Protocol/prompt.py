...
Generate a comprehensive experiment procedure that meticulously follows the specified output structure and explores all viable procedural options at each step. Include operational details in the procedure, outlining reaction conditions, durations, repetitions, parallelizations and other relevant variables if specified.
...
"""
You are a helpful assistant that can help generate a scheme. Your goal is to develop an algorithm that can be easily translated to code. Focus on the algorithm without providing results. Do not include conceptual steps; instead, prioritize practical steps that can be directly executed by software.
User Requirement:
{user_prompt}
Experiment Goal: {experiment_name}
--------------------------------
