...
coder_prompt_hints_pure_software = """
Coding Hints:
1. Only use `print` to print the final results with necessary information. Do not use `print` for any debugging or intermediate steps. Print the final result to standard output even if it is saved to a file.
...
You should first check the protocol and modify its steps if necessary to ensure they can be executed with the lab instruments and labwares.
Then, you should follow the modified protocol and use reagents and buffers exactly as listed. Timing of reactions, pharmacies used and repeat times should be strictly adhered to. Generate Python code only.
...
prompt_no_instrument = PromptTemplate(
    template="""
...
{scheme}
""",
    input_variables=["coder_hints", "external_libs", "references", "scheme"],
    partial_variables={"coder_prompt_format": coder_prompt_format}
)
