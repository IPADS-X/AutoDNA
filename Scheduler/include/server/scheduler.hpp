...
public:
    ...
    bool addWorkflow(std::shared_ptr<Workflow> workflow, bool is_user = true,
                     bool skip_gen_id = false) {
        auto new_workflow_id = 0;
        if (skip_gen_id) {
            new_workflow_id = workflow->getId();
        } else {
            new_workflow_id = getNewWorkflowId(is_user);
            workflow->setId(new_workflow_id);
        }

        // TODO: if pre alloc failed, retry
        // because there are only one mission is checked, so there are not TOCTOU issues
        if (workflow->isPreAlloc()) {
            auto steps = workflow->getSteps();
            for (int i = 0; i < steps.size(); i++) {
                if (!CheckManager::preAllocEquipment(
                        reality_, mac_manager_,
                        std::make_shared<Action>((ActionId)i, steps[i], new_workflow_id), workflow,
                        CheckType::CHECKONLY, [&](std::shared_ptr<Workflow> new_workflow) {
                            return this->addWorkflowAndReOrder(nullptr, new_workflow, true);
                        })) {
                    // here must return and retry 5s later
                    failed_prealloc_workflows_.push(
            ...
            for (int i = 0; i < steps.size(); i++) {
                CheckManager::preAllocEquipment(
                    reality_, mac_manager_,
                    std::make_shared<Action>((ActionId)i, steps[i], new_workflow_id), workflow,
                    CheckType::APPLY, [&](std::shared_ptr<Workflow> new_workflow) {
                        return this->addWorkflowAndReOrder(nullptr, new_workflow, true);
                    });
            }
        }
    ...
    bool canExecute(std::shared_ptr<Action> action) {
        return CheckManager::check(
            reality_, mac_manager_, action, workflows_.at(action->getWorkflowId()),
    ...
    void lockEquipment(std::shared_ptr<Action> action) {
        CheckManager::apply(reality_, mac_manager_, action, workflows_.at(action->getWorkflowId()),
                            [&](std::shared_ptr<Workflow> new_workflow) {
                                return this->addWorkflowAndReOrder(action, new_workflow, false);
                            });
    }
    ...
    void unlockEquipment(std::shared_ptr<Action> action) {
        CheckManager::release(reality_, mac_manager_, action,
                              workflows_.at(action->getWorkflowId()),
                              [&](std::shared_ptr<Workflow> new_workflow) {
                                  return this->addWorkflowAndReOrder(action, new_workflow, false);
                              });
    }
        ...
        if (workflow->isFinished()) {
            ...
            if (workflow->isPreAlloc()) {
                ...
                for (int i = 0; i < steps.size(); i++) {
                    CheckManager::preAllocEquipment(
                        reality_, mac_manager_,
                        std::make_shared<Action>((ActionId)i, steps[i], workflow->getId()),
                        workflow, CheckType::RELEASE, [&](std::shared_ptr<Workflow> new_workflow) {
                            return this->addWorkflowAndReOrder(nullptr, new_workflow, true);
                        });
                }
            }
            ...
            // TODO: ADD MOVE TO WASTE LOGIC
            // CheckManager::moveToWaste(
            //     reality_, mac_manager_, nullptr, workflow, CheckType::APPLY,
            //     [&](std::shared_ptr<Workflow> new_workflow) {
            //         return this->addWorkflowAndReOrder(nullptr, new_workflow, true);
            //     });
        }
    ...
    bool addWorkflowAndReOrder(std::shared_ptr<Action> action, std::shared_ptr<Workflow> workflow,
                               bool is_user = true, bool skip_gen_id = false) {
...
private:
    ...
    const uint64_t prealloc_retry_interval_ms_ = 5000;
