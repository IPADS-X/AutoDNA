...
class AmpAspirateMix {
public:
    enum { kOutput = 0 };
    inline static const std::string Name           = "AmpAspirateMix";
    ...
    static nlohmann::json fromDummy(const DummyStep& dummy_step) {
        ...
        if (dummy_params.contains(std::to_string(static_cast<int>(Dummy::ParamType::POSITION)))) {
            ...
            input[Pos] = (uint16_t)AmplificationArea::AUTO;
        }
...
class AmpAspirateMixStep : public CRTPStep<AmpAspirateMixStep> {
public:
    AmpAspirateMixStep(std::string name, Variables&& user_input = Variables())
        : CRTPStep<AmpAspirateMixStep>(MachineType::AMPLIFICATION, name, std::move(user_input)) {
        step_funcs_ = {std::bind(&AmpAspirateMixStep::phase0, this, std::placeholders::_1,
                                 std::placeholders::_2, std::placeholders::_3),
                       std::bind(&AmpAspirateMixStep::phase1, this, std::placeholders::_1,
                                 std::placeholders::_2, std::placeholders::_3)};
    }
    ...
    phase0(Reality& reality, std::shared_ptr<MachineManager> mac_manager, ActionId action_id) {
        ...
        auto pos              = user_input_[AmpAspirateMix::Pos].get<uint16_t>();
        auto index            = user_input_[AmpAspirateMix::Index].get<uint16_t>();
        auto volume           = user_input_[AmpAspirateMix::Volume].get<uint16_t>();
        auto total            = user_input_[AmpAspirateMix::Total].get<uint16_t>();
        auto num              = user_input_[AmpAspirateMix::PipetteNum].get<uint16_t>();
        auto pipette_tr_index = user_input_[AmpAspirateMix::PipetteTrIndex].get<uint16_t>();
        ...
        if (user_input_.find(AmpAspirateMix::MixSpeed) != user_input_.end()) {
            mix_speed = user_input_[AmpAspirateMix::MixSpeed].get<uint16_t>();
        } else {
            mix_speed = 100;
        }

        if (pos == (uint16_t)AmplificationArea::AUTO) {
            auto tube     = user_input_[AmpAspirateMix::Tube].get<TubeId>();
            ...
            if (pos == (uint16_t)CommonAreaId::NOT_SUCCESS) {
                pos = (uint16_t)AmplificationArea::SAMPLE_AREA;
            }
        }

        logger->info(
            "AmpAspirateMixStep, phase0: pos: {}, index: {}, volume: {}, total: {}, num: {}", pos,
    ...
    phase1(Reality& reality, std::shared_ptr<MachineManager> mac_manager, ActionId action_id) {
        ...
        results.push_back(
            ExecutionResult{next_steps_[AmpAspirateMix::kOutput], Variables(), false});
        return results;
    }
    ...
    getTubeTypeSupportsSteps(Reality& reality, std::map<TubeId, TubeId>& tube_map) {
        ...
        if (user_input_.find(AmpAspirateMix::Tube) == user_input_.end()) {
            return {std::make_shared<AmpAspirateMixStep>(name_, std::move(new_params))};
        }
        auto tube_id  = user_input_[AmpAspirateMix::Tube].get<TubeId>();
        ...
        if (TubeManager::getTubeType(tube_ptr) == TubeType::PCR_TUBE) {
            ...
            new_params[AmpAspirateMix::Tube] = tube_id;
        }

        return {std::make_shared<AmpAspirateMixStep>(name_, std::move(new_params))};
    }

    static std::shared_ptr<AmpAspirateMixStep> fromDummy(DummyStep& dummy_step) {
        if (dummy_step.getType() == Dummy::DummyType::ASPIRATE_MIX) {
            return std::make_shared<AmpAspirateMixStep>(AmpAspirateMix::Name,
                                                        AmpAspirateMix::fromDummy(dummy_step));
        }
        return nullptr;
    }
};

class AmpAspirateMixStage : public TemplatedStage<AmpAspirateMix> {
public:
    AmpAspirateMixStage(std::string id, Variables&& input) : TemplatedStage(std::move(id), true) {
        my_step_ = std::make_shared<AmpAspirateMixStep>(name_, std::move(input));
    }
    ...
    void generateWorkflowHelper(Workflow& workflow) {
        ...
        genWorkflowForNextStage(workflow, AmpAspirateMix::kOutput);
    }
};