...
class FluoMoveCarrier {
public:
    enum { kOutput = 0 };
    inline static const std::string Name     = "FluoMoveCarrier";
    inline static const std::string StartPos = "StartPos";
    inline static const std::string EndPos   = "EndPos";
    inline static const std::string Carrier  = "Carrier";
    inline static const std::string Tube     = Step::Tube;
};

class FluoMoveCarrierStep : public CRTPStep<FluoMoveCarrierStep> {
public:
    FluoMoveCarrierStep(std::string name, Variables&& user_input = Variables())
        : CRTPStep<FluoMoveCarrierStep>(MachineType::FLUORESCENCE, name, std::move(user_input)) {
        step_funcs_ = {std::bind(&FluoMoveCarrierStep::phase0, this, std::placeholders::_1,
                                 std::placeholders::_2, std::placeholders::_3),
                       std::bind(&FluoMoveCarrierStep::phase1, this, std::placeholders::_1,
                                 std::placeholders::_2, std::placeholders::_3)};
    }
    ...
    phase0(Reality& reality, std::shared_ptr<MachineManager> mac_manager, ActionId action_id) {
        ...
        auto start_pos = user_input_[FluoMoveCarrier::StartPos].get<uint16_t>();
        auto end_pos   = user_input_[FluoMoveCarrier::EndPos].get<uint16_t>();

        if (start_pos == (uint16_t)FluorescenceArea::AUTO ||
            end_pos == (uint16_t)FluorescenceArea::AUTO) {
            Carrier* carrier;
            std::shared_ptr<Carrier> carrier_ptr;
            if (user_input_.find(FluoMoveCarrier::Tube) != user_input_.end()) {
                auto tube     = user_input_[FluoMoveCarrier::Tube].get<TubeId>();
                ...
                auto carrierSN = user_input_[FluoMoveCarrier::Carrier].get<std::string>();
            ...
            if (start_pos == (uint16_t)CommonAreaId::NOT_SUCCESS) {
                ...
                carrier->setMachineType((MachineTypeId)MachineType::FLUORESCENCE);
            }
    ...
    phase1(Reality& reality, std::shared_ptr<MachineManager> mac_manager, ActionId action_id) {
        ...
        results.push_back(
            ExecutionResult{next_steps_[FluoMoveCarrier::kOutput], Variables(), false});
        return results;
    }
    ...
    getNeedLockEquipment(Reality& reality, std::shared_ptr<MachineManager> mac_manager_,
        ...
        auto end_pos = user_input_[FluoMoveCarrier::EndPos].get<uint16_t>();
        if (end_pos == (uint16_t)FluorescenceArea::EXIT_AREA) {
    ...
    getNeedUnlockEquipment(Reality& reality, std::shared_ptr<MachineManager> mac_manager_,
        ...
        std::vector<std::pair<MachineType, EquipmentType>> unlocked_equipment = {
            ...
            std::make_pair(machine_type_, EquipmentType::PIPETEE_GUN)};
        auto start_pos = user_input_[FluoMoveCarrier::StartPos].get<uint16_t>();
        if (start_pos == (uint16_t)FluorescenceArea::ENTER_AREA) {
    ...
    getTubeTypeSupportsSteps(Reality& reality, std::map<TubeId, TubeId>& tube_map) {
        ...
        if (user_input_.find(FluoMoveCarrier::Tube) == user_input_.end()) {
            return {std::make_shared<FluoMoveCarrierStep>(name_, std::move(new_params))};
        }
        auto tube_id  = user_input_[FluoMoveCarrier::Tube].get<TubeId>();
        ...
        if (TubeManager::getTubeType(tube_ptr) == TubeType::PCR_TUBE) {
            ...
            new_params[FluoMoveCarrier::Tube] = tube_id;
        }
        return {std::make_shared<FluoMoveCarrierStep>(name_, std::move(new_params))};
    }
    ...
    static std::shared_ptr<Step> fromDummy(TubeId tube_id, AreaId area_id, AreaId from_area_id) {
        ...
        user_input[FluoMoveCarrier::Tube] = tube_id;
        if (from_area_id == (AreaId)CommonAreaId::ENTER_AREA) {
            user_input[FluoMoveCarrier::StartPos] = FluorescenceArea::ENTER_AREA;
        } else if (from_area_id == (AreaId)CommonAreaId::EXIT_AREA) {
            user_input[FluoMoveCarrier::StartPos] = FluorescenceArea::EXIT_AREA;
        } else if (from_area_id == (AreaId)CommonAreaId::AUTO) {
            user_input[FluoMoveCarrier::StartPos] = FluorescenceArea::AUTO;
        } else {
            user_input[FluoMoveCarrier::StartPos] = from_area_id;
        }
        if (area_id == (AreaId)CommonAreaId::ENTER_AREA) {
            user_input[FluoMoveCarrier::EndPos] = FluorescenceArea::ENTER_AREA;
        } else if (area_id == (AreaId)CommonAreaId::EXIT_AREA) {
            user_input[FluoMoveCarrier::EndPos] = FluorescenceArea::EXIT_AREA;
        } else if (area_id == (AreaId)CommonAreaId::AUTO) {
            user_input[FluoMoveCarrier::EndPos] = FluorescenceArea::AUTO;
        } else {
            user_input[FluoMoveCarrier::EndPos] = area_id;
        }

        return std::make_shared<FluoMoveCarrierStep>(FluoMoveCarrier::Name, std::move(user_input));
    }
};

class FluoMoveCarrierStage : public TemplatedStage<FluoMoveCarrier> {
public:
    FluoMoveCarrierStage(std::string id, Variables&& input) : TemplatedStage(std::move(id), true) {
        my_step_ = std::make_shared<FluoMoveCarrierStep>(name_, std::move(input));
    }
    ...
    void generateWorkflowHelper(Workflow& workflow) {
        ...
        genWorkflowForNextStage(workflow, FluoMoveCarrier::kOutput);
    }
};