...
public:
    ...
    inline static const std::string AirGapVolume      = "AirGapVolume";
    inline static const std::string BlowOut           = "BlowOut";
    inline static const std::string AspiratationSpeed = "AspiratationSpeed";
    inline static const std::string DispenseSpeed     = "DispenseSpeed";
    inline static const std::string FreeDispense      = "FreeDispense";
    inline static const std::string TipTouching       = "TipTouching";
    ...
    static nlohmann::json fromDummy(const DummyStep& dummy_step) {
        ...
        if (dummy_params.contains(
                std::to_string(static_cast<int>(Dummy::ParamType::BEFORE_PIPETTE_MIX_TIMES)))) {
            input[BeforeMixTimes] = dummy_params[std::to_string(
                static_cast<int>(Dummy::ParamType::BEFORE_PIPETTE_MIX_TIMES))];
        }

        if (dummy_params.contains(
                std::to_string(static_cast<int>(Dummy::ParamType::BEFORE_PIPETTE_MIX_VOLUME)))) {
            input[BeforeMixVolume] = dummy_params[std::to_string(
                static_cast<int>(Dummy::ParamType::BEFORE_PIPETTE_MIX_VOLUME))];
        }

        if (dummy_params.contains(
                std::to_string(static_cast<int>(Dummy::ParamType::AFTER_PIPETTE_MIX_TIMES)))) {
            input[AfterMixTimes] = dummy_params[std::to_string(
                static_cast<int>(Dummy::ParamType::AFTER_PIPETTE_MIX_TIMES))];
        }

        if (dummy_params.contains(
                std::to_string(static_cast<int>(Dummy::ParamType::AFTER_PIPETTE_MIX_VOLUME)))) {
            input[AfterMixVolume] = dummy_params[std::to_string(
                static_cast<int>(Dummy::ParamType::AFTER_PIPETTE_MIX_VOLUME))];
        }
        ...
        if (dummy_params.contains(
                std::to_string(static_cast<int>(Dummy::ParamType::AIR_GAP_VOLUME)))) {
            input[AirGapVolume] =
                dummy_params[std::to_string(static_cast<int>(Dummy::ParamType::AIR_GAP_VOLUME))];
        }

        if (dummy_params.contains(std::to_string(static_cast<int>(Dummy::ParamType::BLOW_OUT)))) {
            input[BlowOut] =
                dummy_params[std::to_string(static_cast<int>(Dummy::ParamType::BLOW_OUT))];
        } else {
            input[BlowOut] = true;
        }

        if (dummy_params.contains(
                std::to_string(static_cast<int>(Dummy::ParamType::ASPIRATATION_SPEED)))) {
            input[AspiratationSpeed] = dummy_params[std::to_string(
                static_cast<int>(Dummy::ParamType::ASPIRATATION_SPEED))];
        } else {
            input[AspiratationSpeed] = 100;
        }

        if (dummy_params.contains(
                std::to_string(static_cast<int>(Dummy::ParamType::DISPENSE_SPEED)))) {
            input[DispenseSpeed] =
                dummy_params[std::to_string(static_cast<int>(Dummy::ParamType::DISPENSE_SPEED))];
        } else {
            input[DispenseSpeed] = 100;
        }

        if (dummy_params.contains(
                std::to_string(static_cast<int>(Dummy::ParamType::FREE_DISPENSE)))) {
            input[FreeDispense] =
                dummy_params[std::to_string(static_cast<int>(Dummy::ParamType::FREE_DISPENSE))];
        } else {
            input[FreeDispense] = false;
        }

        if (dummy_params.contains(
                std::to_string(static_cast<int>(Dummy::ParamType::TIP_TOUCHING)))) {
            input[TipTouching] =
                dummy_params[std::to_string(static_cast<int>(Dummy::ParamType::TIP_TOUCHING))];
        } else {
            input[TipTouching] = false;
        }
...
public:
    ...
    uint16_t getCurrentNum() const {
        if (user_input_.find(FluoPipette::PipetteNum) != user_input_.end()) {
            return user_input_.at(FluoPipette::PipetteNum).get<uint16_t>();
        }
        return 1;
    }
    ...
    phase0(Reality& reality, std::shared_ptr<MachineManager> mac_manager, ActionId action_id) {
        ...
        auto volume           = user_input_[FluoPipette::Volume].get<uint32_t>();
        ...
        uint16_t air_gap_volume = 0;

        if (user_input_.find(FluoPipette::AirGapVolume) != user_input_.end()) {
            air_gap_volume = user_input_[FluoPipette::AirGapVolume].get<uint16_t>();
        }
        ...
        uint16_t blow_out           = 1;
        uint16_t aspiratation_speed = 100;
        uint16_t dispense_speed     = 100;
        uint16_t free_dispense      = 0;
        uint16_t tip_touching       = 0;

        if (user_input_.find(FluoPipette::BlowOut) != user_input_.end()) {
            blow_out = user_input_[FluoPipette::BlowOut].get<uint16_t>();
        }
        if (user_input_.find(FluoPipette::AspiratationSpeed) != user_input_.end()) {
            aspiratation_speed = user_input_[FluoPipette::AspiratationSpeed].get<uint16_t>();
        }
        if (user_input_.find(FluoPipette::DispenseSpeed) != user_input_.end()) {
            dispense_speed = user_input_[FluoPipette::DispenseSpeed].get<uint16_t>();
        }
        if (user_input_.find(FluoPipette::FreeDispense) != user_input_.end()) {
            free_dispense = user_input_[FluoPipette::FreeDispense].get<uint16_t>() ? 1 : 0;
        }
        if (user_input_.find(FluoPipette::TipTouching) != user_input_.end()) {
            tip_touching = user_input_[FluoPipette::TipTouching].get<uint16_t>() ? 1 : 0;
        }
        ...
        logger->debug("Pipetting from {}:{} to {}:{} with volume {} and {} times before mixing, {} times after mixing, air gap volume {}, blow out {}, aspiratation speed {}, dispense speed {}, free dispense {}, tip touching {}",
                      magic_enum::enum_name((FluorescenceArea)start_pos), start_index,
                      magic_enum::enum_name((FluorescenceArea)end_pos), end_index, volume,
                      before_mix_time, after_mix_time, air_gap_volume, blow_out, aspiratation_speed,
                      dispense_speed, free_dispense, tip_touching);
        ...
        machine->pipette(start_pos, start_index, volume, end_pos, end_index, num, pipette_tr_index,
                         ...
                         mix_speed, air_gap_volume, blow_out, aspiratation_speed, dispense_speed,
                         free_dispense, tip_touching, action_id);
