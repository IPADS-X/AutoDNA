...
class FluoAspirateMix {
public:
    enum { kOutput = 0 };
    inline static const std::string Name           = "FluoAspirateMix";
    ...
    static nlohmann::json fromDummy(const DummyStep& dummy_step) {
        ...
        if (dummy_params.contains(std::to_string(static_cast<int>(Dummy::ParamType::POSITION)))) {
            ...
            input[Pos] = (uint16_t)FluorescenceArea::AUTO;
        }
...
class FluoAspirateMixStep : public CRTPStep<FluoAspirateMixStep> {
public:
    FluoAspirateMixStep(std::string name, Variables&& user_input = Variables())
        : CRTPStep<FluoAspirateMixStep>(MachineType::FLUORESCENCE, name, std::move(user_input)) {
        step_funcs_ = {std::bind(&FluoAspirateMixStep::phase0, this, std::placeholders::_1,
                                 std::placeholders::_2, std::placeholders::_3),
                       std::bind(&FluoAspirateMixStep::phase1, this, std::placeholders::_1,
                                 std::placeholders::_2, std::placeholders::_3)};
    }
    ...
    phase0(Reality& reality, std::shared_ptr<MachineManager> mac_manager, ActionId action_id) {
        ...
        auto pos              = user_input_[FluoAspirateMix::Pos].get<uint16_t>();
        auto index            = user_input_[FluoAspirateMix::Index].get<uint16_t>();
        auto volume           = user_input_[FluoAspirateMix::Volume].get<uint16_t>();
        auto total            = user_input_[FluoAspirateMix::Total].get<uint16_t>();
        auto num              = user_input_[FluoAspirateMix::PipetteNum].get<uint16_t>();
        auto pipette_tr_index = user_input_[FluoAspirateMix::PipetteTrIndex].get<uint16_t>();
        ...
        if (user_input_.find(FluoAspirateMix::MixSpeed) != user_input_.end()) {
            mix_speed = user_input_[FluoAspirateMix::MixSpeed].get<uint16_t>();
        } else {
            mix_speed = 100;
        }

        if (pos == (uint16_t)FluorescenceArea::AUTO) {
            auto tube     = user_input_[FluoAspirateMix::Tube].get<TubeId>();
            ...
            if (pos == (uint16_t)CommonAreaId::NOT_SUCCESS) {
                pos = (uint16_t)FluorescenceArea::SAMPLE_AREA;
            }
        }

        logger->info(
            "FluoAspirateMixStep, phase0: pos: {}, index: {}, volume: {}, total: {}, num: {}", pos,
    ...
    phase1(Reality& reality, std::shared_ptr<MachineManager> mac_manager, ActionId action_id) {
        ...
        results.push_back(
            ExecutionResult{next_steps_[FluoAspirateMix::kOutput], Variables(), false});
        return results;
    }
    ...
    getTubeTypeSupportsSteps(Reality& reality, std::map<TubeId, TubeId>& tube_map) {
        ...
        if (user_input_.find(FluoAspirateMix::Tube) == user_input_.end()) {
            return {std::make_shared<FluoAspirateMixStep>(name_, std::move(new_params))};
        }
        auto tube_id  = user_input_[FluoAspirateMix::Tube].get<TubeId>();
        ...
        if (TubeManager::getTubeType(tube_ptr) == TubeType::PCR_TUBE) {
            ...
            new_params[FluoAspirateMix::Tube] = tube_id;
        }
        return {std::make_shared<FluoAspirateMixStep>(name_, std::move(new_params))};
    }

    static std::shared_ptr<FluoAspirateMixStep> fromDummy(DummyStep& dummy_step) {
        if (dummy_step.getType() == Dummy::DummyType::ASPIRATE_MIX) {
            return std::make_shared<FluoAspirateMixStep>(FluoAspirateMix::Name,
                                                         FluoAspirateMix::fromDummy(dummy_step));
        }
        return nullptr;
    }
};

class FluoAspirateMixStage : public TemplatedStage<FluoAspirateMix> {
public:
    FluoAspirateMixStage(std::string id, Variables&& input) : TemplatedStage(std::move(id), true) {
        my_step_ = std::make_shared<FluoAspirateMixStep>(name_, std::move(input));
    }
    ...
    void generateWorkflowHelper(Workflow& workflow) {
        ...
        genWorkflowForNextStage(workflow, FluoAspirateMix::kOutput);
    }
};