...
public:
    ...
    phase0(Reality& reality, std::shared_ptr<MachineManager> mac_manager, ActionId action_id) {
        ...
        if (end_index == (uint16_t)CommonIndexId::AUTO &&
            ...
            if ((unsigned short)(IndexId)CommonIndexId::NOT_SUCCESS == end_index) {
                // prealloc workflow will use all available positions
                end_index = 0;
            }
    ...
    std::string getOperationName() const override { return "MoveTube"; }
    ...
    static std::shared_ptr<PuriMoveTubeStep> fromDummy(DummyStep& dummy_step) {
        if (dummy_step.getType() == Dummy::DummyType::MOVE_TUBE ||
            dummy_step.getType() == Dummy::DummyType::MOVE_PCR_TUBE) {
