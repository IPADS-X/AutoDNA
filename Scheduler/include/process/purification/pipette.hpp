...
public:
    enum { kOutput = 0 };
    inline static const std::string Name            = "PuriPipette";
...
class PuriPipetteStep : public CRTPStep<PuriPipetteStep> {
public:
    PuriPipetteStep(std::string name, Variables&& user_input = Variables())
        : CRTPStep<PuriPipetteStep>(MachineType::PURIFICATION, name, std::move(user_input)) {
        step_funcs_ = {std::bind(&PuriPipetteStep::phase0, this, std::placeholders::_1,
                                 std::placeholders::_2, std::placeholders::_3),
                       std::bind(&PuriPipetteStep::phase1, this, std::placeholders::_1,
                                 std::placeholders::_2, std::placeholders::_3)};
    }

    bool setCurrentNum(uint16_t current_num) override {
        if (current_num <= 0 || current_num > 4) {
            logger->error("PuriPipetteStep, setCurrentNum: current_num {} is out of range [1, 8]",
                          current_num);
            return false;
        }

        if (user_input_.find(PuriPipette::PipetteNum) != user_input_.end()) {
            user_input_[PuriPipette::PipetteNum] = current_num;
            if (user_input_[PuriPipette::StartPos].get<uint16_t>() ==
                (uint16_t)PurificationArea::SAMPLE_AREA) {
                user_input_[PuriPipette::StartIndex] = workflow_id_ - 1;
            } else if (user_input_[PuriPipette::EndPos].get<uint16_t>() ==
                       (uint16_t)PurificationArea::SAMPLE_AREA) {
                user_input_[PuriPipette::EndIndex] = workflow_id_ - 1;
            }
            return true;
        }

        logger->error("PuriPipetteStep, setCurrentNum: user_input_ does not contain PipetteNum");
    ...
    phase0(Reality& reality, std::shared_ptr<MachineManager> mac_manager, ActionId action_id) {
        ...
        logger->info("PuriPipetteStep, phase0: start_pos: {}, start_index: {}, volume: {}, "
        ...
        if (volume <= 5000) {
            ...
            SPDLOG_ASSERT(pipette_tr_index == (uint16_t)PipetteTrType::UL_200,
                          "PuriPipetteStep, phase0: volume > 5000, but pipette_tr_index != UL_200");
        }

        uint16_t before_mix_time   = 0;
        uint16_t before_mix_volume = 0;
        if (user_input_.find(PuriPipette::BeforeMixTimes) != user_input_.end() &&
            user_input_.find(PuriPipette::BeforeMixVolume) != user_input_.end()) {
            before_mix_time   = user_input_[PuriPipette::BeforeMixTimes].get<uint16_t>();
            before_mix_volume = user_input_[PuriPipette::BeforeMixVolume].get<uint16_t>();
        }

        uint16_t after_mix_time   = 0;
        uint16_t after_mix_volume = 0;
        if (user_input_.find(PuriPipette::AfterMixTimes) != user_input_.end() &&
            user_input_.find(PuriPipette::AfterMixVolume) != user_input_.end()) {
            after_mix_time   = user_input_[PuriPipette::AfterMixTimes].get<uint16_t>();
            after_mix_volume = user_input_[PuriPipette::AfterMixVolume].get<uint16_t>();
        }
        ...
        if (user_input_.find(PuriPipette::MixSpeed) != user_input_.end()) {
            mix_speed = user_input_[PuriPipette::MixSpeed].get<uint16_t>();
        } else {
            mix_speed = 100;
        }

        if (start_pos == (uint16_t)PurificationArea::AUTO) {
            auto tube = user_input_[PuriPipette::SrcTube].get<TubeId>();
            auto tube_pos =
        ...
        if (end_pos == (uint16_t)PurificationArea::AUTO) {
            auto tube     = user_input_[PuriPipette::DstTube].get<TubeId>();
        ...
        if (start_pos == (uint16_t)PurificationArea::AUTO &&
            ...
            auto start_tube = reality.getTube(user_input_[PuriPipette::SrcTube].get<TubeId>());
            auto end_tube   = reality.getTube(user_input_[PuriPipette::DstTube].get<TubeId>());
        ...
        logger->info("PuriPipetteStep, phase0 after auto adjust: start_pos: {}, start_index: {}, "
                     "volume: {}, end_pos: {}, end_index: {}, num: {}",
                     start_pos, start_index, volume, end_pos, end_index, num);
    ...
    getTubeTypeSupportsSteps(Reality& reality, std::map<TubeId, TubeId>& tube_map) {
        ...
        if (user_input_.find(PuriPipette::SrcTube) == user_input_.end() ||
            user_input_.find(PuriPipette::DstTube) == user_input_.end()) {
            return {std::make_shared<PuriPipetteStep>(name_, std::move(new_params))};
        }
        auto dst_tube_id  = user_input_[PuriPipette::DstTube].get<TubeId>();
        ...
        if (TubeManager::getTubeType(dst_tube_ptr) == TubeType::PCR_TUBE) {
            ...
            new_params[PuriPipette::DstTube] = dst_tube_id;
            auto volume                      = user_input_[PuriPipette::Volume].get<int>() / 8;
            for (int i = 0; i < 8; i++) {
                new_params[PuriPipette::EndIndex] = i;
                new_params[PuriPipette::Volume]   = volume;
                res.push_back(std::make_shared<PuriPipetteStep>(name_, std::move(new_params)));
            }
            return res;
        }
        return {std::make_shared<PuriPipetteStep>(name_, std::move(new_params))};
    ...
    static std::shared_ptr<PuriPipetteStep> fromDummy(DummyStep& dummy_step) {
        if (dummy_step.getType() == Dummy::DummyType::PIPETTE) {
            return std::make_shared<PuriPipetteStep>(PuriPipette::Name,
                                                     PuriPipette::fromDummy(dummy_step));
        }
        return nullptr;
    }
};

class PuriPipetteStage : public TemplatedStage<PuriPipette> {
public:
    PuriPipetteStage(std::string id, Variables&& input) : TemplatedStage(std::move(id), true) {
        my_step_ = std::make_shared<PuriPipetteStep>(name_, std::move(input));
    }
    ...
    void generateWorkflowHelper(Workflow& workflow) {
        ...
        genWorkflowForNextStage(workflow, PuriPipette::kOutput);
    }
};