    ...
    transferSelfBlock(Reality& reality, std::shared_ptr<MachineManager> mac_manager_,
        ...
        for (const auto& workflow : sources) {
            ...
            for (int i = 0; i < steps.size(); i++) {
                ...
                if (i + 1 < steps.size()) {
                    ...
                    if (dummy_step->getType() == Dummy::DummyType::PIPETTE &&
                        ...
                        if (dummy_step->getParams().at(std::to_string(
                                static_cast<int>(Dummy::ParamType::DEST_TUBE_INDEX))) !=
                                next_dummy_step->getParams().at(std::to_string(
                                    static_cast<int>(Dummy::ParamType::TUBE_INDEX))) ||
                            dummy_step->getParams().at(
                                std::to_string(static_cast<int>(Dummy::ParamType::END_INDEX))) !=
                                next_dummy_step->getParams().at(
                                    std::to_string(static_cast<int>(Dummy::ParamType::INDEX)))) {
                            logger_->debug(
                                "pipette and mix tube not match {}({}), {}({})",
                                ...
                                dummy_step->getParams()
                                    .at(std::to_string(
                                        static_cast<int>(Dummy::ParamType::END_INDEX)))
                                    .get<IndexId>(),
                                next_dummy_step->getParams()
                                    .at(std::to_string(
                                        static_cast<int>(Dummy::ParamType::TUBE_INDEX)))
                                    .get<TubeId>(),
                                next_dummy_step->getParams()
                                    .at(std::to_string(static_cast<int>(Dummy::ParamType::INDEX)))
                                    .get<IndexId>());
            ...
            for (int i = 0; i < step_groups.size(); i++) {
                // if contains capping, capped tube
                if (auto dummy_step =
                        dynamic_cast<DummyStep*>(step_groups[i]->steps_.front().get())) {
                    auto params = dummy_step->getParams();
                    if (dummy_step->getType() == Dummy::DummyType::CAP &&
                        params.find(std::to_string(
                            static_cast<int>(Dummy::ParamType::TUBE_LIST))) != params.end()) {
                        auto tube_id_list = params.at(
                            std::to_string(static_cast<int>(Dummy::ParamType::TUBE_LIST)));
                        for (const auto& tube_id : tube_id_list) {
                            logger_->debug("Mark tube {} as capped", tube_id.get<TubeId>());
                            Simulator::markCapped(tube_id);
                        }
                    }
                }
                ...
                for (const auto& step : step_groups[start_i]->steps_) {
                    logger_->debug(
                        "Adding step of type {} to new workflow",
                        magic_enum::enum_name(dynamic_cast<DummyStep*>(step.get())->getType()));
