...
public:
    ...
    static const inline std::map<std::string, Dummy::ParamType> param_type_map_ = {
        ...
        {"before_pipette_mix_times", Dummy::ParamType::BEFORE_PIPETTE_MIX_TIMES},
        {"after_pipette_mix_times", Dummy::ParamType::AFTER_PIPETTE_MIX_TIMES},
        {"before_pipette_mix_volume", Dummy::ParamType::BEFORE_PIPETTE_MIX_VOLUME},
        {"after_pipette_mix_volume", Dummy::ParamType::AFTER_PIPETTE_MIX_VOLUME},
        {"air_gap_volume", Dummy::ParamType::AIR_GAP_VOLUME},
        {"blow_out", Dummy::ParamType::BLOW_OUT},
        {"aspiratation_speed", Dummy::ParamType::ASPIRATATION_SPEED},
        {"dispense_speed", Dummy::ParamType::DISPENSE_SPEED},
        {"tip_touching", Dummy::ParamType::TIP_TOUCHING},
        {"free_dispense", Dummy::ParamType::FREE_DISPENSE},
...
public:
    ...
    transferValue(Reality& reality, Dummy::DummyType type, Dummy::ParamType key,
        ...
        if (key == Dummy::ParamType::BEFORE_PIPETTE_MIX_VOLUME ||
            key == Dummy::ParamType::AFTER_PIPETTE_MIX_VOLUME ||
            key == Dummy::ParamType::AIR_GAP_VOLUME) {
            int real_volume = value.get<float>() * 100;
            return {{std::to_string(static_cast<int>(key)), real_volume}};
        }

        if (key == Dummy::ParamType::ASPIRATATION_SPEED ||
            key == Dummy::ParamType::DISPENSE_SPEED) {
            int real_value = std::stoi(std::to_string(value.get<int>()));
            return {{std::to_string(static_cast<int>(key)), real_value}};
        }

        if (key == Dummy::ParamType::BLOW_OUT) {
            bool real_value = value.get<int>() != 0;
            return {{std::to_string(static_cast<int>(key)), real_value}};
        }

        if (key == Dummy::ParamType::TIP_TOUCHING) {
            bool real_value = value.get<int>() != 0;
            return {{std::to_string(static_cast<int>(key)), real_value}};
        }

        if (key == Dummy::ParamType::FREE_DISPENSE) {
            bool real_value = value.get<int>() != 0;
            return {{std::to_string(static_cast<int>(key)), real_value}};
        }
