        ...
        for (const auto& source : sources) {
            ...
            for (const auto& step : steps) {
                ...
                if (last_step && ((dynamic_cast<LibPipetteStep*>(last_step.get()) != nullptr &&
                    ...
                    auto last_tube_id                        = last_step_params[LibPipette::DstTube]
                                                                  .get<TubeId>();
                    auto current_tube_id                     = new_params[LibAspirateMix::Tube]
                                                                  .get<TubeId>();
                    auto last_volume                        = last_step_params[LibPipette::Volume]
                                                                  .get<uint16_t>();
                    auto mix_total_volume                   = new_params[LibAspirateMix::Volume].get<uint16_t>();
                    if (last_tube_id != current_tube_id) {
                    ...
                    if (last_volume < 5000 && mix_total_volume > 5000) {
                        // do not combine if last volume will use small tip
                        new_workflow->addStep(last_step);
                        last_step = step;
                        jump      = false;
                        continue;
