...
public:
    ...
    IndexId allocEquipment(EquipmentType equipment, CheckType checkType,
                           std::shared_ptr<Workflow> original_workflow, std::shared_ptr<Step> step,
                           std::shared_ptr<Container> container, bool is_all);

    bool releaseEquipment(EquipmentType equipment, std::shared_ptr<Workflow> original_workflow,
                          std::shared_ptr<Step> step, std::shared_ptr<Container> container,
                          bool is_all);
    ...
    allocAConsumeTube(std::shared_ptr<Container> tube) {
        for (const auto& [area_id, carrier] : place_area_carriers_) {
            if (carrier->isConsumer()) {
                ...
                if (index != -1) {
                    logger->debug("Allocated tube {} to carrier {} in index {}",
                                  TubeManager::getTubeId(tube), carrier->getName(), index);
    ...
    bool addPipetteTr(PipetteTrType tr_type, int num) {
        std::lock_guard<std::mutex> lock(mtx_);
        available_pipette_trs_[tr_type] = std::max(0, available_pipette_trs_[tr_type]);
        available_pipette_trs_[tr_type] += num;
        available_pipette_trs_[tr_type] =
            std::min(available_pipette_trs_[tr_type], DEFAULT_PIPETTE_TR_COUNTS);
        return true;
    }

    bool consumePipetteTr(PipetteTrType tr_type, int num) {
        std::lock_guard<std::mutex> lock(mtx_);
        if (available_pipette_trs_[tr_type] >= num) {
            available_pipette_trs_[tr_type] -= num;
            return true;
        }
        return false;
...
protected:
    ...
    std::map<PipetteTrType, int> available_pipette_trs_ = {
        {PipetteTrType::UL_50, DEFAULT_PIPETTE_TR_COUNTS},
        {PipetteTrType::UL_200, DEFAULT_PIPETTE_TR_COUNTS},
    };
