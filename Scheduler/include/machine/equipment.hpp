...
public:
    ...
    IndexId allocAll(std::shared_ptr<Workflow> workflow = nullptr) {
        ...
        alloc_slot_num_        = max_slot_num_;
        single_alloc_workflow_ = workflow;
        return 0;
    }

    IndexId releaseAll(std::shared_ptr<Workflow> workflow = nullptr) {
        if (isEmpty() || single_alloc_workflow_ != workflow) {
            return (IndexId)CommonIndexId::NOT_SUCCESS;
        }
        alloc_slot_num_ = 0;
        equipment_steps_.clear();
