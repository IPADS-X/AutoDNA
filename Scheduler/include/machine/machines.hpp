...
public:
    FluorescenceModbusMachine(const char* ip, int port)
        : ModbusMachine(MachineType::FLUORESCENCE, ip, port,
                        {Equipment::build(EquipmentType::ROBOT_ARM),
                         ...
                         Equipment::build(EquipmentType::EXIT_POS)},
                        {(AreaId)FluorescenceArea::SAMPLE_AREA,
                         (AreaId)FluorescenceArea::PCR_STRIP_SAMPLE_CONSUMABLE_AREA}) {}
    ...
    void pipette(uint16_t start_pos, uint16_t start_index, uint16_t volume, uint16_t end_pos,
                 ...
                 uint16_t mix_speed, uint16_t air_gap_volume, uint16_t blow_out,
...
public:
    PurificationModbusMachine(const char* ip, int port)
        : ModbusMachine(
              ...
              {Equipment::build(EquipmentType::ROBOT_ARM),
               Equipment::build(EquipmentType::PIPETEE_GUN),
               Equipment::build(EquipmentType::CENTRIFUGE, 3),
               Equipment::build(EquipmentType::MAGNETIC_RACK, 96),
               Equipment::build(EquipmentType::HEATER_SHAKER, 96), Equipment::build(EquipmentType::PCR, 96),
               Equipment::build(EquipmentType::ENTER_POS),
    ...
    LibraryModbusMachine(const char* ip, int port)
        : ModbusMachine(
              ...
              {Equipment::build(EquipmentType::ROBOT_ARM),
               ...
               Equipment::build(EquipmentType::EXIT_POS), Equipment::build(EquipmentType::TIMER)},
              {(AreaId)LibraryArea::NORMAL_TEMP_REAGENT_AREA,
               (AreaId)LibraryArea::PCR_STRIP_CONSUMABLE_AREA, (AreaId)LibraryArea::SAMPLE_AREA,
               (AreaId)LibraryArea::COLD_LABEL_AREA, (AreaId)LibraryArea::COLD_REAGENT_AREA}) {}
