...
Before running the application, you must configure reagents in the JSON configuration files located in the `config/` directory.
...
This will be generated by AutoDNA-python.
...
Note: This can be generated by AutoDNA-python.

1. Create new reagent configuration files in `config/`
2. Define protocol flow steps in corresponding `protocol_flow_*.json` files
3. Update workflow definitions in the source code
4. Test with local build mode before production deployment

### Testing

Use the local build mode for development and testing:
```bash
make
./local_build/bin/main_local_web ./config
```
...
For detailed debugging, use the local build with debug symbols:
```bash
make
./local_build/bin/main_local_web ./config
```
...
4. Test thoroughly in local mode before submitting changes
