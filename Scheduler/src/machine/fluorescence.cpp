...
void FluorescenceModbusMachine::pipette(uint16_t start_pos, uint16_t start_index, uint16_t volume,
                                        ...
                                        uint16_t air_gap_volume, uint16_t blow_out,
                                        uint16_t aspiratation_speed, uint16_t dispense_speed,
                                        uint16_t free_dispense, uint16_t tip_touching,
                                        ActionId action_id) {
    ...
    rc = client_.writeSingleRegister(FluoPipetteModbus::AIR_GAP_VOLUME_ADDR, air_gap_volume);
    SPDLOG_ASSERT(rc == 1, "");

    rc = client_.writeSingleRegister(FluoPipetteModbus::BLOW_OUT_ADDR, blow_out);
    SPDLOG_ASSERT(rc == 1, "");

    rc = client_.writeSingleRegister(FluoPipetteModbus::ASPIRATATION_SPEED_ADDR, aspiratation_speed);
    SPDLOG_ASSERT(rc == 1, "");

    rc = client_.writeSingleRegister(FluoPipetteModbus::DISPENSE_SPEED_ADDR, dispense_speed);
    SPDLOG_ASSERT(rc == 1, "");

    rc = client_.writeSingleRegister(FluoPipetteModbus::FREE_DISPENSE_ADDR, free_dispense);
    SPDLOG_ASSERT(rc == 1, "");

    rc = client_.writeSingleRegister(FluoPipetteModbus::TIP_TOUCHING_ADDR, tip_touching);
    SPDLOG_ASSERT(rc == 1, "");
...
void FluorescenceModbusMachine::time(uint32_t duration, ActionId action_id) {
