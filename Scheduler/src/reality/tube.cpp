...
bool TubeManager::isUselessTube(std::shared_ptr<Container> tube) {
    auto location = getTubePosition(TubeManager::getTubeId(tube), TubePositionType::WHOLE_TUBE);

    auto area_type = MachineManager::fromAreaId(location.machine_type, location.area_id);

    if (area_type != magic_enum::enum_name(PurificationArea::WASTE_AREA)) {
        return true;
    }
...
void TubeManager::Print() {
    std::cout << "TubeManager:" << std::endl;
    std::cout << "  Strip Tubes:" << std::endl;
    for (const auto& [name, tube] : strip_tubes_) {
        std::cout << "    Name: " << name << ", ID: " << tube->getId()
                  << ", Label: " << tube->getLabel() << "Location: ("
                  << magic_enum::enum_name(tube->getNowTubePosition().machine_type) << ", "
                  << MachineManager::fromAreaId(tube->getNowTubePosition().machine_type,
                                                tube->getNowTubePosition().area_id)
                  << ", " << tube->getNowTubePosition().index << ")" << std::endl;
    }
    std::cout << "  PCR Tubes:" << std::endl;
    ...
    std::cout << "  Chamber Tubes:" << std::endl;
