...
bool Reality::renewPipettes(MachineTypeId                   machine_type,
                            std::shared_ptr<MachineManager> mac_manager) {
    auto machine = mac_manager->getMachine<Machine>(machine_type);
    if (machine) {
        machine->addPipetteTr(PipetteTrType::UL_50, DEFAULT_PIPETTE_TR_COUNTS);
        machine->addPipetteTr(PipetteTrType::UL_200, DEFAULT_PIPETTE_TR_COUNTS);
        return true;
    }
    return false;
}
